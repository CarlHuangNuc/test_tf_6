...
class Qwen2_5OmniThinkerForConditionalGeneration(Qwen2_5OmniPreTrainedModelForConditionalGeneration, GenerationMixin):
    ...
    def streaming_prefill(
        ...
        if self.llm_past_key_values is not None:
            ...
            ## carl 20160104
            import copy
            self.llm_past_key_values = copy.deepcopy(self.llm_past_key_values_for_system[self.system_cache_num])
    ...
    def forward(
        ...
        if self.system_cache_num >= 0:
            cache_length_system = self.llm_past_key_values_for_system[self.system_cache_num][0][0].shape[2]
        ### carl add 20260104 for no system cache
        else:
            cache_length_system = 0
        ...
        if self.use_stream:
            ...
            cache_length = cache_length_system

        attention_mask = torch.ones((attention_mask.shape[0], cache_length + inputs_embeds.shape[1]), dtype=torch.int, device=self.device)
        if self.use_stream or self.system_cache_num >=0:
